...
namespace OnLineStore
{
	...
	{	
		public SqlConnection connection=new SqlConnection(ConfigurationSettings.AppSettings["ConStr"].ToString());
		private string str;
		...
		{
			str="select * from Customer WHERE RegistrationDate>'"+constraint+"'";
		...
		public void fillsearchddl(string field,string table,string order,DataSet dataset)
		{
			str="SELECT "+field+" FROM "+table+" WHERE HeadID IS NOT NULL ORDER BY "+order+" DESC;";
			select=new SqlCommand(str,connection);
			SqlDataAdapter dataadapter=new SqlDataAdapter();
			dataadapter.SelectCommand=select;
			try
		...
		public void insert1(string tabelname,string field,string values)
		{
			str="INSERT INTO "+tabelname+" ("+field+")VALUES ("+values+");";
			SqlCommand insertcommand=new SqlCommand(str,connection);
			try
